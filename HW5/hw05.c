    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        while (operation[j] != ';' || (i == 2)) { // || operation[j] == '\0'
            ...
            if (operation[j] == '-' || operation[j] == '+') {
                if ((operation[j] == '-') && (rNum1 == -1)){ //determines whether to use addi for constants
                    printf("sub ");
                } else if ((operation[j] == '+') && (rNum1 == -1)) { //only right number must not exist for add
                    printf("add ");
                } else if ((operation[j] == '-') || (operation[j] == '+')) { //right number must be constant 
                    printf("addi ");
                }
